...
def receive_broadcast_thread():
    ...
    while True:
        ...
        if(newnodeflag == 1):
            tcpClient = daemon_thread_builder(
                exchange_timestamps_thread, args=(parsed[0], "127.0.0.1", parsed[2],))
            tcpClient.start()
            tcpClient.join()
        elif(newnodeflag == 2):
            x = neighbor_information.get(parsed[0])
            neighbor_information.update({parsed[0]: NeighborInfo(
                x.delay, x.last_timestamp, x.broadcast_count+1)})
